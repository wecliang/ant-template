/**
 * 命名规则判断
 * 数字 字符 和 _ 组成， 且数字不能开头
 */
export const isStr = (name:string) => {
 ...
 * 字符串首字母转大写
 */
export const initialToUpper = (str:string) => {
  if(!str) return '';
 ...
 * @param len 随机生成函数数量
 */
export const getRandomChineseWord = (len:number | [number, number] = [6, 12]) => {
  var _rsl = "";
  var _randomUniCode = "";
  if(Array.isArray(len)){
    len = Math.round(Math.random() * (len[1] - len[0]) + len[0]);
  }
  for(let i=0; i<len; i++){
    _randomUniCode += `\\u${Math.floor(Math.random() * (40870 - 19968) + 19968).toString(16)}`;
  }
  eval(`_rsl = "${_randomUniCode}"`);
  return simplized(_rsl);
}
 ...
 * @param len 随机生成字符串长度
 */
export const getRandomString = (len:number | [number, number] = [16, 32]) => { 
  if(Array.isArray(len)){
    len = Math.round(Math.random() * (len[1] - len[0]) + len[0]);
  }
